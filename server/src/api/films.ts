 ...
 *
 * Instant film is measured per shot, as /api/prices ranks it: a pack is 8 shots and a
 * multipack multiplies that, so the lowest *ticket* price would chart the cheapest
 * single pack and hide a five-pack that is better value, and a multipack coming into
 * stock would read as a price change. `unit` tells the client which measure it got.
 */
filmsRouter.get("/:id/price-history", async (req, res) => {
  ...
  const filmResult = await db.query<{ format: string }>(
    db.dialect === "postgres"
      ...
      : "SELECT format FROM films WHERE id = ?",
    [filmId]
  );
  const unit: "ticket" | "per_shot" =
    filmResult.rows[0]?.format === "instant" ? "per_shot" : "ticket";
  // Same per-shot measure as the instant ordering in api/prices.ts.
  const priceExpr =
    unit === "per_shot"
    ...
    const historyResult = await db.query<{ day: string; min_price_cad_cents: number }>(
      ...
      FROM (
        SELECT
          ...
          ROUND(MIN(${priceExpr}))::integer AS min_price_cad_cents
  ...
  const historyResult = await db.query<{ day: string; min_price_cad_cents: number }>(
    ...
    SELECT
      ...
      CAST(ROUND(MIN(${priceExpr})) AS INTEGER) AS min_price_cad_cents
    FROM price_snapshots ps
    JOIN listings l ON l.id = ps.listing_id
    WHERE l.film_id = ?
