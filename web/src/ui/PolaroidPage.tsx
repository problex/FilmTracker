...
export function PolaroidPage() {
  ...
  // The API reports instant film history per shot, matching the figure the table leads on.
  useEffect(() => {
    setHistory(null);
    ...
    (async () => {
      try {
        ...
        const body = (await res.json()) as { points: PriceHistoryPoint[] };
        if (!cancelled) setHistory(body.points ?? []);
      } catch (e) {
                        ...
                        <div className="card filmDetailCard">
                          <div className="detailHeader">
                            <div>
                              ...
                              <div className="subtitle">
                                Cheapest {hideOutOfStock ? "in-stock " : ""}offer each day, per
                                shot — multipacks included
                              </div>
                            </div>
                          ...
                          {history && <PriceHistoryChart points={history} unit="per_shot" />}
                        </div>
                      </td>
                    </tr>
                  )}
